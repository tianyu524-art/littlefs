...
static int collect_lschk_entries_legacy(
        sim_state_t *sim, const char *path, lschk_entry_t **entries_out, size_t *count_out) {
    lfs_dir_t dir;
    int err = lfs_dir_open(&sim->lfs, &dir, path);
    if (err) {
...
static int cmd_lschk_legacy(sim_state_t *sim, int argc, char **argv) {
    if (ensure_mounted(sim)) {
        return -1;
    }
...
static int cmd_lsrepair_legacy(sim_state_t *sim, int argc, char **argv) {
    if (ensure_mounted(sim)) {
        return -1;
    }
